...
(function(){
  var stringify, ref$, filter, map, foldl, each, uniqueBy, plus, minus, take, reverse, times, div, fromHex, $toHex, get, post, Web3, Tx, BN, hdkey, bip39, jsonParse, deadline, bignumber, getEthereumFullpairByIndex, getGasEstimate, calcFee, getKeys, round, toHex, prepareInternalTxs, transformInternalTx, up, transformTx, getInternalTransactions, getTransactionInfo, getTransactions, getDec, calcGasPrice, tryGetLateest, getNonce, isAddress, createTransaction, checkDecodedData, pushTx, checkTxStatus, getTotalReceived, getUnconfirmedBalance, getBalance, getMarketHistoryPrices, out$ = typeof exports != 'undefined' && exports || this, slice$ = [].slice, toString$ = {}.toString;
  stringify = require('qs').stringify;
  const TRANSACTIONS_PER_QUERY = 15;
  let cache = {
    transactions: {}
  };
  ref$ = require('prelude-ls'), filter = ref$.filter, reverse = ref$.reverse, take = ref$.map, map = ref$.map, foldl = ref$.foldl, each = ref$.each, uniqueBy = ref$.uniqueBy;
  ...
  transformInternalTx = curry$(function(network, type, t){
    ...
    time = t.timeStamp;
    url = url + "/tx/" + tx;
    ref1$ = t != null ? (ref$ = t.moreInfo) != null ? ref$.info : void 8 : void 8, gasUsed = ref1$ ? ref1$.gasUsed: "0", cumulativeGasUsed = ref1$.cumulativeGasUsed, effectiveGasPrice = ref1$.effectiveGasPrice, status = ref1$.status;
    const data = t.moreInfo;
    gasUsed = data.info.gasUsed;
    cumulativeGasUsed = data.info ? data.info.cumulativeGasUsed : "0";
    effectiveGasPrice = data.info ? data.info.effectiveGasPrice : "0";
    status = t.status;
  ...
  getInternalTransactions = function(config, cb){
    ...
    return get(apiUrl + "?" + query).timeout({
      ...
      return jsonParse(resp.text, function(err, result){
        ...
        const internalTxs = reverse((result.result || []).splice(-15));

        return prepareInternalTxs(network, internalTxs, function(err){
          var txs, e;
          try {
  ...
  out$.getTransactionInfo = getTransactionInfo = function(config, cb){
    ...
    query = [tx];
    const hash = tx;
    if (hash && cache.transactions[`${hash}`] != null) {
      return cb(null, cache.transactions[`${hash}`]);
    }
    return makeQuery(network, 'eth_getTransactionReceipt', query, function(err, tx){
      ...
      cache.transactions[`${hash}`] = result;
      return cb(null, result);
    });
  };
  out$.getTransactions = getTransactions = function(arg$, cb){
    ...
    return get(apiUrl + "?" + query).timeout({
      ...
      return jsonParse(resp.text, function(err, result){
        ...
        const externalTxs = (result.result || []).splice(-TRANSACTIONS_PER_QUERY);
