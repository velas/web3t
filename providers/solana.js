...
(function(){
  var moment, ref$, map, pairsToObj, foldl, any, each, find, sum, filter, head, values, join, reverse, uniqueBy, sortBy, get, post, plus, minus, div, times, BitcoinLib, bip39, jsonParse, deadline, decode, guid, BN, bs58, Buffer, tweetnacl, solanaWeb3, lo, assert, bip32, findMax, calcFee, makeQuery, toBase58, toBuffer, getAccountInfo, getKeys, extend, getDec, addAmount, parseRateString, extractVal, parseResult, getDepositAddressInfo, getError, getRecentBlockhash, getAccount, freeOwnership, swapNativeToEvmData, swapNativeToEvm, createTransaction, pushTx, getTotalReceived, getUnconfirmedBalance, getBalance, getApiUrl, checkTxStatus, getTransactions, prepareRawTxs, prepareTxs, getSentAmount, getAction, isValidAddress, getTransactionInfo, out$ = typeof exports != 'undefined' && exports || this, slice$ = [].slice, toString$ = {}.toString;
  ...
  prepareTxs = function(network, arg$, address, cb){
    ...
    return makeQuery(network, 'getConfirmedTransaction', [signature, 'jsonParsed'], function(err, data){
      var t, txData, ref$, fee, status, transaction, accountKeys, instructions, senders, sender, receiverIndex, _receivers, receiverObj, receiver, value, time, dec, url, cluster, customUrl, $cluster, uri, type, recipientType, _tx;
      if (err != null) {
      ...
      if (err == null) {
        ...
        accountKeys);
        receiverObj = (function(){
          switch (false) {
          case _receivers.length !== 1:
            return _receivers[0];
          case !(_receivers.length >= 1):
            t = filter(function(it){
              return it.pubkey.indexOf("EvmState") < 0;
            })(
            _receivers);
            if (t.length > 0) {
              return t[0];
        ...
        receiver = (function(){
          var ref$, ref1$, ref2$;
          switch (false) {
          case !((ref$ = instructions[0]) != null && ((ref1$ = ref$.parsed) != null && ((ref2$ = ref1$.info) != null && ref2$.destination))):
            return instructions[0].parsed.info.destination;
          case !(receiverObj != null && receiverObj.pubkey):
            return receiverObj.pubkey;
          default:
            return "";
          }
        }());
        value = (function(){
          var ref$, ref1$, ref2$;
          switch (false) {
          case !((ref$ = instructions[0]) != null && ((ref1$ = ref$.parsed) != null && ((ref2$ = ref1$.info) != null && ref2$.lamports))):
            return instructions[0].parsed.info.lamports;
          default:
            return getSentAmount(txData)[receiver];
          }
        }());
