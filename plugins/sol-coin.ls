(->
  mainnetConfig = void
  testnetConfig = void
  testnet2Config = void
  mainnet = void
  testnet = void
  ref$ = void
  ref1$ = void
  color = void
  type = void
  enabled = void
  name = void
  token = void
  nickname = void
  image = void
  usdInfo = void
  out$ = typeof exports isnt 'undefined' && exports || this
  mainnetConfig = {
    decimals: 8
      txFee: '0.0014'
      txFeeOptions: {
        auto: '0.0020'
        cheap: '0.0020'
      }
      messagePrefix: 'Ethereum'
      mask: '3000000000000000000000000000000000'
      api: {
        provider: 'solana'
        web3Provider: 'http://localhost:8899/rpc'
        url: 'http://localhost:8899/'
        apiUrl: 'http://localhost:8899/api'
      }
  }
  testnetConfig = {
    decimals: 8
    txFee: '0.0014'
    txFeeOptions: {
      auto: '0.0020'
      cheap: '0.0020'
    }
    messagePrefix: 'Ethereum'
    mask: '3000000000000000000000000000000000'
    api: {
      provider: 'solana'
      web3Provider: 'https://explorer.next.velas.com/rpc'
      url: 'http://localhost:8899/'
      apiUrl: 'http://localhost:8899/api'
      cluster: 'custom'
      customUrl: 'https://explorer.testnet.velas.com/rpc' 
    }
  }
  out$.mainnet = mainnet = mainnetConfig
  out$.testnet = testnet = if (if typeof window isnt 'undefined' && window isnt null
    if (ref$ = window.location) isnt null then if (ref1$ = ref$.href) isnt null then ref1$.indexOf 'testnet2' else void else void
  else
    void) > -1 then testnet2Config else testnetConfig
  out$.color = color = '#9E4FEB'
  out$.type = type = 'coin'
  out$.enabled = enabled = true
  out$.name = name = 'Velas Native'
  out$.token = token = 'vlx'
  out$.nickname = nickname = 'solana'
  out$.image = image = 'data:image/png;base64,'
  out$.usdInfo = usdInfo = 'url(url(https://explorer.velas.com/ticker).price_usd'
  return ).call this